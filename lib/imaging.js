...
module.exports = function(Cam) {
   ...
   */
	...
	/**
	 * Moves the focus lens in an absolute, a relative or in a continuous manner from its current
	 * position
	 * @param {object} options
	 * @param {Object} options.token videoSourceToken
	 * @param {object} options.focus Content of the requested move (focus) operation
	 * @param {object} options.focus.absolute Parameters for the absolute focus control
	 * @param {number} options.focus.absolute.position Position parameter for the absolute focus control
	 * @param {number} options.focus.absolute.speed Speed parameter for the absolute focus control
	 * @param {object} options.focus.relative Parameters for the relative focus control
	 * @param {number} options.focus.relative.distance Distance parameter for the relative focus control
	 * @param {number} options.focus.relative.speed Speed parameter for the relative focus control
	 * @param {object} options.focus.continuous Parameter for the continuous focus control
	 * @param {number} options.focus.continuous.speed Speed parameter for the Continuous focus control
	 * @param {Cam~RequestCallback} [callback]
	 */
	Cam.prototype.moveImaging = function(options, callback) {
		this._request({
			service: 'ptz'
			, body: this._envelopeHeader() +
					...
					(options.focus ? (
						...
						(options.focus.relative ? (
							'<Continuous xmlns="http://www.onvif.org/ver10/schema">' +
							'<Distance xmlns="http://www.onvif.org/ver10/schema">' +
							options.focus.relative.distance +
							'</Distance>' +
							(options.focus.relative.speed ? (
								'<Speed xmlns="http://www.onvif.org/ver10/schema">' +
								options.focus.relative.speed +
								'</Speed>'
							) : '') +
						...
						(options.focus.continuous ? (
							'<Continuous xmlns="http://www.onvif.org/ver10/schema">' +
							'<Speed xmlns="http://www.onvif.org/ver10/schema">' +
							options.focus.continuous.speed +
							'</Speed>' +
							'</Continuous>'
						) : '') +
						'</Focus>'
